...
namespace :db do
  ...
  task :bulk_seed, [:num] => :environment do |t, args|
    ...
    binding.pry
    Tire.index(CommentService.config[:es_index][:comment_thread]).delete
    CommentThread.create_elasticsearch_index
    Tire.index(CommentService.config[:es_index][:comment_thread]) { import CommentThread.all }
  end
  ...
  task :seed => :environment do
    ...
    Notification.delete_all
    Subscription.delete_all
    Tire.index CommentService.config[:es_index][:comment_thread] do delete end
    CommentThread.create_elasticsearch_index
